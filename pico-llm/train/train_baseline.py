...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train baseline softmax Transformer on synthetic numeric sequences.")
    ...
    parser.add_argument("--seq_types", type=str, default="counting,arithmetic,geometric,alternating,random_walk",
                        help="Comma-separated sequence families to mix.")
    parser.add_argument("--max_steps", type=int, default=-1, help="Optional early stop per epoch for quick tests.")
...
def main():
    ...
    history = train_model(model, model_cfg, "baseline", train_loader, val_loader, out_dir)
    ...
    print(f"[baseline] test_loss={test_loss:.4f} test_acc={test_acc:.3f}")
